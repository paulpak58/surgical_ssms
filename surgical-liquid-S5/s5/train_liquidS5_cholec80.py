...
def train(args):
    ...
    if args.split_val_test:
        ...
        print(f'Video split 40/40 for training and test')
        train_labels_80 = np.array(train_labels_80)
        train_num_each_80 = np.array(train_num_each_80)
        train_start_vidx = np.array(train_start_vidx)
        g_LFB_train = np.array(g_LFB_train)
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_acc:
            ...
            if args.save_weights:
                ...
                # save_checkpoint(ckpt_dir=ckpt_path, target={'model': state}, step=step, overwrite=True, keep=1)
                params = {'params':state.params,'opt_state':state.opt_state}
...
if __name__=='__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='./best_state', help='dir to save state')
