...
#from flax.training import train_state, checkpoint
import orbax
...
def train(args):
    ...
    checkpoint_manager = CheckpointManager('./best_s5_official/checkpoint_1080', {'model': PyTreeCheckpointer()}, options)
    
    # Restore model
    # target = {'model': model_cls, 'state': state}
    # new_state = restore_checkpoint(ckpt_dir='/ppak/surgical_ncp/src/cholec_pipeline/surgical-liquid-S5/s5/best_liquidS5_weights_flax_step',target=None, step=None)  
    new_state = restore_checkpoint(ckpt_dir='/ppak/surgical_ncp/src/cholec_pipeline/surgical-liquid-S5/s5/best_s5_official/checkpoint_1080/',target=None, step=None)  
    new_state = new_state['model']
    print(new_state.keys())
    # state = flax.serialization.from_state_dict(target=state, state=new_state)

    # path = 'checkpoint_'+step
    # state = checkpoint_manager.restore(step)['model']
    # step = checkpoint_manager.latest_step()
    # state = checkpoint_manager.restore(step)
    state = new_state['opt_state']
    model_params = new_state['params']
    print(model_params.keys())
    # restore_checkpoint('best_liquidS5/orbax/1/default',target=state,step=step)
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_acc:
            ...
            if args.save_weights:
                ...
                print(type(restored_state))
                #checkpoint_manager.save(step,state,save_kwargs={'save_args':save_args})
                # save_checkpoint(ckpt_path, ckpt, step=0, overwrite=True, keep=1)
                print(f'Done')


                """
                save_state = train_state.TrainState.create(apply_fn=model_cls.apply, params=state.params, tx=state.tx, step=state.step)
                ckpt_path = './ckpts'
                checkpoints.save_checkpoint(ckpt_path, save_state, epoch)
                """
