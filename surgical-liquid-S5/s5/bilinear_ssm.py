...
def apply_ssm(Lambda,
    ...
    """ Compute the LxH output of discretized SSM given an LxH input.
        Args:
            Lambda (complex64): diagonal state matrix                   (P,)
            E_tilde (complex64): bilinear matrix                         (P,H)
            B_tilde      (complex64): input matrix                       (P, H)
    ...
    Eus = jax.vmap(lambda u: E_tilde @ u)(input_sequence)

    Lambdas = Lambda + Eus  # (L, P)
    if clip_eigs:
        Lambdas = np.clip(Lambdas.real, None, -1e-4) + 1j * Lambdas.imag

    @jax.vmap
    def _do_vmapped_discretize(Lambda):
        # TODO: Maybe just do Euler update on B matrix
    ...
    Lambda_elements, B_bars = _do_vmapped_discretize(Lambdas)
...
class S5BilinearSSM(nn.Module):
    ...
    def setup(self):
        ...
        #Initialize bilinear matrix E
        E_init = normal(stddev=0.5 ** 0.5)
        self.E = self.param("E", E_init, (self.P, self.H, 2))
        self.E_tilde = self.E[..., 0] + 1j * self.E[..., 1]
