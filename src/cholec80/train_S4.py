...
for epoch in range(max_epochs):
    ...
    with torch.no_grad():
        for i in test_we_use_start_idx_80:
            ...
            # print(f'TCN INPUT SHAPE {video_fe.shape}')
            out_features = model.forward(video_fe)[-1]
            # print(f'TCN OUTPUT SHAPE {out_features.shape}')
            out_features = out_features.squeeze(1)
            # print(f'TRANS_SVNET INPUT SHAPE {out_features.shape}')
            p_classes1 = model1(out_features, long_feature)
