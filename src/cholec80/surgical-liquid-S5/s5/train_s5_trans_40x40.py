...
def eval_step(batch_inputs, batch_labels, batch_integration_timesteps, state, model, batchnorm, params, weighted_ce=False):
    if batchnorm:
        logits = model.apply({"params": params, "batch_stats": state.batch_stats}, batch_inputs, batch_integration_timesteps)
    else:
        logits = model.apply({"params": params}, batch_inputs, batch_integration_timesteps)
...
def train_s5_trans(args):
    ...
    params = {'params':None, 'opt_state':None}
    ...
    # val_labels_80 = np.concatenate((np.array(val_labels_80), np.array(test_labels_80)), axis=0)
    '''
    val_labels_80 = val_labels_80 + test_labels_80
    val_num_each_80 = np.concatenate((np.array(val_num_each_80), np.array(test_num_each_80)), axis=0)
    test_start_vidx = np.array(test_start_vidx) + np.array(val_num_each_80).sum()
    val_start_vidx = np.concatenate((np.array(val_start_vidx), np.array(test_start_vidx)), axis=0)
    g_LFB_val = np.concatenate((np.array(g_LFB_val), np.array(g_LFB_test)), axis=0)
    test_we_use_start_idx_80 = [x for x in range(0)]
    '''
    ...
    for epoch in range(max_epochs):
        ...
        for i in train_we_use_start_idx_80:
            ...
            # Evaluation step
            loss, acc, pred = eval_step(inputs, labels, integration_times, state, model_cls, args.batchnorm, model_params)
        ...
        total_accuracy_phase = (float(val_corrects_phase)+float(test_corrects_phase)) / (len(val_labels_80)+len(test_labels_80))
        ...
        if total_acccuracy_phase > best_total_acccuracy_phase:
            best_total_accuracy_phase = total_accuracy_phase
            ...
            best_val_all_preds_phase = val_all_preds_phase

            best_all_preds = best_val_all_preds_phase + best_test_all_preds_phase
    ...
    if not os.path.exists("./eval"):
        os.makedirs("./eval")
    pred_name = './eval/best_s5_trans_40x40.pkl'
    with open(pred_name, 'wb') as f:
        # pickle.dump(best_test_all_preds_phase, f)
        pickle.dump(best_all_preds, f)
    '''
    ...
    torch.save(best_model_wts, "./best_model/S4/" + model_name + "_weights_40_40" + ".pth")
